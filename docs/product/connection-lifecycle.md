...
When an SSH transport disconnects, existing Local Endpoints remain allocated so other processes cannot claim their ports, but new client connections fail promptly rather than queueing indefinitely. Reconnection uses exponential backoff with jitter. Authentication and host-key failures suspend automatic retries until the user acts. Listener cleanup remains suspended until a successful reconnect produces a complete observation.
