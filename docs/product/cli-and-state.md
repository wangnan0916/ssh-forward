...
`ssh-forward ui start` is a second per-user background process: it binds loopback HTTP, records `ui.pid` / `ui.url` / `ui.log` next to the manager files, and talks to the Manager through `app.Connect`. Autospawn encodes serve options as environment (`SSH_FORWARD_UI_SERVE=1`, host, policies path, optional SSH config, and `SSH_FORWARD_CONFIG_DIR`); the child enters `app.ServeUI` without parsing a Cobra command tree. Foreground `ssh-forward ui serve` still uses Cobra. `ui stop` ends only that process. `SSH_FORWARD_UI_BINARY` overrides the spawned UI executable; `SSH_FORWARD_UI_NO_OPEN=1` skips opening a browser. `app.StartUI` / `ServeUI` / `StopUI` / `LiveUIURL` own that lifecycle the same way `Connect` / `Serve` / `Stop` own the manager.
