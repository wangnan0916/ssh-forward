...
- `manager.execute` — translate one wire command into `Manager.Execute`. Initial command kinds are `manual_forward.add` (`operation_id`, SSH-alias `host`, `remote_port`, and `family`) and `manual_forward.remove` (`operation_id` and `forward_id`). Outcomes carry the resulting revision and complete Forward.
- `manager.snapshot` — return a complete scoped Snapshot. The initial all-host request is `{"scope":{"kind":"all"}}`; a new Manager returns `{"snapshot":{"revision":0,"hosts":[]}}`. Host entries carry alias, Connection State, Discovery State/Capability, baseline and scanner identity, complete deterministically ordered Listener Observations with bounded Process Chains, and complete Forwards.
