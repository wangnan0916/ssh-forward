...
- `manager.snapshot` — return a complete scoped Snapshot. The initial all-host request is `{"scope":{"kind":"all"}}`; a new Manager returns `{"snapshot":{"revision":0,"hosts":[]}}`. Host entries carry alias, connection state, and complete deterministically ordered Forwards.
