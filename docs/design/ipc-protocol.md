...
- `manager.execute` — translate one wire command into `Manager.Execute`. Initial command kinds are `manual_forward.add` (`operation_id`, SSH-alias `host`, `remote_port`, and `family`) and `manual_forward.remove` (`operation_id` and `forward_id`); the slice-5 policy surface adds `policy.approve` and `policy.suppress` (`operation_id`, `host`, `remote_port`, optional `family`), which record a One-time Approval or One-time Suppression for the current Listener Lifetime — Approval immediately creates the Listener's Managed Forward. Outcomes carry the resulting revision and complete Forward.
- `manager.snapshot` — return a complete scoped Snapshot. The initial all-host request is `{"scope":{"kind":"all"}}`; a new Manager returns `{"snapshot":{"revision":0}}`. The single `host` entry (present once a Development Host is configured) carries alias, Connection State, Discovery State/Capability with a `diagnostic` explaining partiality or failure (`scanner_reported_partial`, `process_metadata_unavailable`, `evidence_truncated`, a scanner failure reason, or `observation_resync`), baseline and scanner identity, complete deterministically ordered Listener Observations with bounded Process Chains, per-listener `listener_lifetimes` verdicts (with `post_baseline` set for Listeners first observed after the Discovery Baseline), `ask_listeners` (Listeners currently needing a user decision: post-baseline, unsuppressed, and not matched automatically), and complete Forwards (Kind `manual` or `managed`).
