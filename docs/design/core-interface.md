...
- **IPC — external Adapter:** `app.Connect` / `app.Serve` own the per-user singleton lifecycle (env-encoded child, not Cobra argv). `jsonrpc` is one deep module: Unix socket Listen/Dial/Serve plus a remote `core.Manager`. Pinned `jrpc2` owns generic RPC mechanics; the Adapter adds a regular integer version call, bounded line framing, and Watch-to-notification mapping. CLI `--json` and JSON-RPC encode the same tagged `core.Snapshot`; there is no duplicate wire DTO. Client and server share one framing implementation. A named-pipe adapter is not split out until Windows Local Machine is in scope. CLI human status uses `NewDocument`; Policy Evidence stays on those rows, not on the Manager Snapshot. Addable is computed inside `NewDocument` (loopback Available that is not Ignore and not already Auto-forward).
