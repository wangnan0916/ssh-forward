...
Policy reconciliation has two wakes. One internal `decide` step turns observations, policies, and hysteresis into create/remove intent; `allocateIntent` runs Allocate outside the Manager lock; `applyIntent` commits policy, the Forward table, and Local Port Conflicts under the lock, then Close after Unlock. Non-conflict allocation failures retry on the next observation without publishing. The observation path keeps two-generation hysteresis: a Managed Forward is created after two consecutive Auto-forward matches and removed after two consecutive observations that no longer match. A listener that leaves `desired` (gone, or no longer Auto-forward) clears its create count, so a later reappearance is a fresh first generation rather than completing a count from before the gap. A saved policy edit (including a change to Ignore) applies against the current observations immediately and resets hysteresis; production Managers poll the policy source every 250ms so CLI `add`/`remove` take effect without waiting for the next scan. A no-change policy poll is a no-op so the ticker cannot fill hysteresis. A Policy Diagnostic-only poll publishes without touching hysteresis. Local Port Conflict from allocation is recorded on the Snapshot until a later allocation succeeds or the listener is no longer desired.
...
- **IPC — external Adapter:** `app.Connect` / `app.Serve` own the per-user singleton lifecycle; `app.StartUI` / `ServeUI` / `StopUI` / `LiveUIURL` own the loopback WebUI process (env-encoded child, not Cobra argv). `jsonrpc` is one module: Unix socket Listen/Dial/Serve plus the JSON-RPC session. Hello is negotiated outside jrpc2; afterwards Dial and Serve share the jrpc2 session. It embeds Snapshots through `cli/internal/snapshot`. CLI `--json` uses that same package and does not import the RPC adapter. Wire DTOs remain separate from domain types. Client and server share one framing module (`Send` / `Recv` / `Close`, with a response-written hook after handshake). A named-pipe adapter is not split out until Windows Local Machine is in scope. CLI human status and the WebUI share `present.NewDocument`; Policy Evidence stays on those rows, not on the Manager Snapshot. Addable is computed inside `NewDocument` (loopback Available that is not Ignore and not already Auto-forward). The WebUI's policy seam is `ui.Intent` (production adapter: `app.FilePolicyReader`).
