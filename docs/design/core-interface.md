...
- **IPC — external Adapter:** `app.Connect` / `app.Serve` own the per-user singleton lifecycle (env-encoded child, not Cobra argv). `jsonrpc` is one deep module: Unix socket Listen/Dial/Serve plus a remote `core.Manager`. Pinned `jrpc2` owns generic RPC mechanics; the Adapter adds a regular integer version call, bounded line framing, wire DTOs, and Watch-to-notification mapping. It embeds Snapshots through `cli/internal/snapshot`; CLI `--json` uses that same package and does not import the RPC adapter. Client and server share one framing implementation. A named-pipe adapter is not split out until Windows Local Machine is in scope. CLI human status uses `NewDocument`; Policy Evidence stays on those rows, not on the Manager Snapshot. Addable is computed inside `NewDocument` (loopback Available that is not Ignore and not already Auto-forward).
