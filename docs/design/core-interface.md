...
- **IPC — external Adapter:** `app.Connect` / `app.Serve` own the per-user singleton lifecycle; the Unix-socket adapter dials and listens; `jsonrpc` embeds Snapshots through `cli/internal/snapshot`. CLI `--json` uses that same package and does not import the RPC adapter. Wire DTOs remain separate from domain types. Client and server share one framing module (`Send` / `Recv` / `Close`, with a response-written hook after handshake).
